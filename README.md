...
ルート周辺検索マップ - Google Maps APIを活用したルート上の店舗・施設検索アプリ

## 概要

出発地と目的地を入力し、「ラーメン」「コンビニ」「電気屋」など自由なキーワードでルート上またはルート付近にある店舗・施設を検索できる地図アプリです。検索結果の店舗を選択すると、その店舗を経由した新しいルートを再計算し、元のルートとの差分時間・距離・到着予想時刻を表示します。
...
4. **検索範囲を調整**: スライダーで100m〜2000mの範囲を設定
